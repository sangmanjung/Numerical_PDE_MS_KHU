%% Cyclic game coexistence paper : ERPS game on PDE
    ...
    for j = 2:m-1
        for k = 2:l-1
            ...
            a(j,k,i+1)=a(j,k,i)+dt*M*((a(j+1,k,i)-2*a(j,k,i)+a(j-1,k,i))/h^2 + (a(j,k+1,i)-2*a(j,k,i)+a(j,k-1,i))/h^2)+...
                dt*mu*a(j,k,i)*(1-(a(j,k,i)+b(j,k,i)+c(j,k,i)+d(j,k,i)+e(j,k,i)))-dt*sigma*e(j,k,i)*a(j,k,i)-dt*Pa/2*a(j,k,i)*a(j,k,i);
            
            b(j,k,i+1)=b(j,k,i)+dt*M*((b(j+1,k,i)-2*b(j,k,i)+b(j-1,k,i))/h^2 + (b(j,k+1,i)-2*b(j,k,i)+b(j,k-1,i))/h^2)+...
                dt*mu*b(j,k,i)*(1-(a(j,k,i)+b(j,k,i)+c(j,k,i)+d(j,k,i)+e(j,k,i)))-dt*sigma*a(j,k,i)*b(j,k,i)-dt*Pb/2*b(j,k,i)*b(j,k,i);
            
            c(j,k,i+1)=c(j,k,i)+dt*M*((c(j+1,k,i)-2*c(j,k,i)+c(j-1,k,i))/h^2 + (c(j,k+1,i)-2*c(j,k,i)+c(j,k-1,i))/h^2)+...
                dt*mu*c(j,k,i)*(1-(a(j,k,i)+b(j,k,i)+c(j,k,i)+d(j,k,i)+e(j,k,i)))-dt*sigma*b(j,k,i)*c(j,k,i)-dt*Pc/2*c(j,k,i)*c(j,k,i);
            
            d(j,k,i+1)=d(j,k,i)+dt*M*((d(j+1,k,i)-2*d(j,k,i)+d(j-1,k,i))/h^2 + (d(j,k+1,i)-2*d(j,k,i)+d(j,k-1,i))/h^2)+...
                dt*mu*d(j,k,i)*(1-(a(j,k,i)+b(j,k,i)+c(j,k,i)+d(j,k,i)+e(j,k,i)))-dt*sigma*c(j,k,i)*d(j,k,i)-dt*Pd/2*d(j,k,i)*d(j,k,i);
            
            e(j,k,i+1)=e(j,k,i)+dt*M*((e(j+1,k,i)-2*e(j,k,i)+e(j-1,k,i))/h^2 + (e(j,k+1,i)-2*e(j,k,i)+e(j,k-1,i))/h^2)+...
                dt*mu*e(j,k,i)*(1-(a(j,k,i)+b(j,k,i)+c(j,k,i)+d(j,k,i)+e(j,k,i)))-dt*sigma*d(j,k,i)*e(j,k,i)-dt*Pe/2*e(j,k,i)*e(j,k,i);
        end
    end
end
