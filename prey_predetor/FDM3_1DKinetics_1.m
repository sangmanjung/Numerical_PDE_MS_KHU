...
for i=1:n-1
    for j=2:m-1
        ...
        % Kinetics (i)
        u(i+1,j)=u(i,j)+dt*(u(i,j-1)-2*u(i,j)+u(i,j+1))/h^2+dt*u(i,j)*(1-u(i,j))-dt*v(i,j)*u(i,j)/(u(i,j)+alpha);
        v(i+1,j)=v(i,j)+dt*delta*(v(i,j-1)-2*v(i,j)+v(i,j+1))/h^2+dt*beta*u(i,j)*v(i,j)/(u(i,j)+alpha)-dt*gamma*v(i,j);
